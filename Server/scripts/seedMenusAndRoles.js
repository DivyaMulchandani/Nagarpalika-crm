...
const RoleSchema = new mongoose.Schema(
  { roleName: { type: String, required: true, unique: true, trim: true }, isActive: { type: Boolean, default: true } },
  { timestamps: true }
);
...
const EmployeeRolesSchema = new mongoose.Schema(
  {
    ...
    roles: [{
      ...
      read: Boolean, write: Boolean, delete: Boolean, edit: Boolean, print: Boolean, mail: Boolean,
    }],
    isActive: { type: Boolean, default: true },
  },
  { timestamps: true }
...
const MENU_STRUCTURE = [
  ...
  {
    group: { name: "Setup", icon: "ri-settings-3-line", isLink: false, sequence: 2 },
    menus: [
  ...
  {
    group: { name: "Location Master", icon: "ri-map-pin-2-line", isLink: false, sequence: 3 },
    menus: [
  ...
  {
    group: { name: "Master Data", icon: "ri-list-settings-line", isLink: true, menuUrl: "/master-data", sequence: 4 },
    menus: [],
  },
  {
    group: { name: "Notifications", icon: "ri-notification-3-line", isLink: false, sequence: 5 },
    menus: [
      { name: "WhatsApp Messages", url: "/whatsapp",       sequence: 1 },
      { name: "Email Setup",       url: "/email-setup",    sequence: 2 },
      { name: "Email Types",       url: "/email-for",      sequence: 3 },
      { name: "Email Templates",   url: "/email-template", sequence: 4 },
    ],
  },
  {
    group: { name: "Access Control", icon: "ri-shield-keyhole-line", isLink: false, sequence: 6 },
    menus: [
  ...
  {
    group: { name: "Reports", icon: "ri-bar-chart-box-line", isLink: true, menuUrl: "/reports", sequence: 7 },
    menus: [],
  },
];
...
async function seed() {
  ...
  const toClean = ["employees", "sessions", "counters", "otps", "whatsappmessages", "whatsappconfigs"];
  console.log("=== Step 1: Clean test data ===");
  for (const col of toClean) {
    try {
      const count = await db.collection(col).countDocuments();
      if (count > 0) { await db.collection(col).deleteMany({}); console.log(`  Cleared ${count} from ${col}`); }
      else console.log(`  ${col}: empty`);
    } catch { console.log(`  ${col}: not found`); }
  }

  // Step 2 — Clear menus, roles, permissions
  console.log("\n=== Step 2: Clear menus/roles ===");
  console.log(`  Deleted ${(await Menu.deleteMany({})).deletedCount} menus`);
  console.log(`  Deleted ${(await MenuGroup.deleteMany({})).deletedCount} groups`);
  console.log(`  Deleted ${(await EmployeeRoles.deleteMany({})).deletedCount} permission sets`);
  ...
  for (const entry of MENU_STRUCTURE) {
    ...
    for (const menu of entry.menus) {
      const menuDoc = await Menu.create({
        menuName: menu.name, menuGroup: groupDoc._id, menuUrl: menu.url,
        sequence: menu.sequence, isActive: true, isParent: false, parentMenu: null,
      });
  ...
  for (const roleName of ROLES) {
    ...
    const roles = allUrls.map((url) => {
      ...
      const perms = permConfig._allMenus ? permConfig._allMenus : (permConfig[url] || NO_ACCESS);
      return { menuId: ids.menuId || null, menuGroupId: ids.menuGroupId, ...perms };
    });

    await EmployeeRoles.create({ roleId: roleMap[roleName]._id, roles, isActive: true });
    const readable = roles.filter((r) => r.read).length;
    console.log(`  [Permissions] ${roleName}: ${readable}/${allUrls.length} menus accessible`);
  }
