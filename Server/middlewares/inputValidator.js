...
const requiredPhoneValidator = (fieldName) => body(fieldName)
    .trim()
    .notEmpty().withMessage(`${fieldName} is required`)
    .isLength({ max: MAX_LENGTHS.PHONE }).withMessage(`${fieldName} must not exceed ${MAX_LENGTHS.PHONE} characters`)
    .matches(/^[+\d\s\-()]+$/).withMessage(`${fieldName} contains invalid characters`);

const optionalMongoIdBody = (fieldName) => body(fieldName)
...
const optionalLongText = (fieldName, max = MAX_LENGTHS.LONG_TEXT) => body(fieldName)
    .optional({ nullable: true })
    .isLength({ max }).withMessage(`${fieldName} must not exceed ${max} characters`);

const patientCommonValidators = [
    optionalNameValidator('lastName'),
    phoneValidator('alternateMobile'),
    optionalEmail,
    optionalMongoIdBody('titleId'),
    optionalMongoIdBody('bloodGroupId'),
    optionalMongoIdBody('maritalStatusId'),
    optionalMongoIdBody('occupationId'),
    optionalMongoIdBody('idProofTypeId'),
    optionalMongoIdBody('referralSourceId'),
    optionalMongoIdBody('referredByPatient'),
    optionalMongoIdBody('referredByDoctor'),
    body('idProofNumber').optional({ nullable: true }).trim().isLength({ max: 50 }),
    optionalLongText('medicalHistory'),
    optionalLongText('currentMedications'),
    optionalLongText('notes'),
    body('dateOfBirth').optional({ nullable: true }).isISO8601().withMessage('dateOfBirth must be a valid date'),
    body('weddingAnniversary').optional({ nullable: true }).isISO8601().withMessage('weddingAnniversary must be a valid date'),
    body('status').optional({ nullable: true }).isIn(['partial', 'active', 'inactive']).withMessage('status must be partial|active|inactive'),
    booleanValidator('isActive'),
    body('address').optional({ nullable: true }).isObject(),
    body('address.line1').optional({ nullable: true }).trim().isLength({ max: MAX_LENGTHS.SHORT_TEXT }),
    body('address.line2').optional({ nullable: true }).trim().isLength({ max: MAX_LENGTHS.SHORT_TEXT }),
    body('address.pincode').optional({ nullable: true }).trim().isLength({ max: 10 }).matches(/^[\d-]*$/).withMessage('pincode contains invalid characters'),
    optionalMongoIdBody('address.countryId'),
    optionalMongoIdBody('address.stateId'),
    optionalMongoIdBody('address.cityId'),
    body('emergencyContact').optional({ nullable: true }).isObject(),
    body('emergencyContact.name').optional({ nullable: true }).trim().isLength({ max: MAX_LENGTHS.NAME }),
    body('emergencyContact.phone').optional({ nullable: true }).trim().isLength({ max: MAX_LENGTHS.PHONE }).matches(/^[+\d\s\-()]*$/).withMessage('emergencyContact.phone contains invalid characters'),
    optionalMongoIdBody('emergencyContact.relationshipId'),
    body('allergies').optional({ nullable: true }).isArray().withMessage('allergies must be an array'),
    body('allergies.*.allergyTypeId').optional({ nullable: true }).isMongoId().withMessage('allergies.*.allergyTypeId must be a valid id'),
    body('allergies.*.notes').optional({ nullable: true }).isLength({ max: 1000 }),
];

export const createPatientValidation = [
    nameValidator('firstName'),
    requiredPhoneValidator('mobileNumber'),
    mongoIdValidator('genderId', 'body'),
    ...patientCommonValidators,
    handleValidationErrors,
];

export const updatePatientValidation = [
    mongoIdValidator('patientId', 'param'),
    body('firstName').optional({ nullable: true }).trim().isLength({ min: MIN_LENGTHS.NAME, max: MAX_LENGTHS.NAME }),
    body('mobileNumber').optional({ nullable: true }).trim().isLength({ max: MAX_LENGTHS.PHONE }).matches(/^[+\d\s\-()]+$/).withMessage('mobileNumber contains invalid characters'),
    optionalMongoIdBody('genderId'),
    ...patientCommonValidators,
    handleValidationErrors,
];

export const quickCreatePatientValidation = [
    nameValidator('firstName'),
    requiredPhoneValidator('mobileNumber'),
    mongoIdValidator('genderId', 'body'),
    handleValidationErrors,
...
export const documentIdParamValidation = [
    mongoIdValidator('documentId', 'param'),
    handleValidationErrors,
];

export const duplicateCheckValidation = [
    query('mobile').optional().trim().isLength({ max: MAX_LENGTHS.PHONE }).matches(/^[+\d\s\-()]*$/).withMessage('mobile contains invalid characters'),
    query('firstName').optional().trim().isLength({ max: MAX_LENGTHS.NAME }),
    query('lastName').optional().trim().isLength({ max: MAX_LENGTHS.NAME }),
    handleValidationErrors,
];

export const documentUploadFieldValidation = [
    mongoIdValidator('patientId', 'param'),
    optionalMongoIdBody('categoryId'),
    handleValidationErrors,
];

export const allowedPatientFields = [
    'titleId', 'firstName', 'lastName', 'dateOfBirth', 'genderId',
    'bloodGroupId', 'maritalStatusId', 'occupationId',
    'mobileNumber', 'alternateMobile', 'email',
    'address', 'idProofTypeId', 'idProofNumber',
    'allergies', 'medicalHistory', 'currentMedications',
    'weddingAnniversary', 'emergencyContact',
    'referralSourceId', 'referredByPatient', 'referredByDoctor',
    'status', 'notes', 'isActive',
];

export const allowedQuickPatientFields = ['firstName', 'mobileNumber', 'genderId'];

export const allowedPatientSearchFields = [
    'skip', 'per_page', 'sorton', 'sortdir', 'match', 'isActive', 'isDeleted',
];
