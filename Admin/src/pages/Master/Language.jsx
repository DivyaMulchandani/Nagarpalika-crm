...
import { searchLanguages, deleteLanguage } from "../../api/languages.api";
...
const Language = () => {
  ...
  const [column, setColumn] = useState("order");
  ...
  const fetchItems = async () => {
    ...
    if (skip < 0) skip = 0;
    await searchLanguages({ skip, per_page: perPage, sorton: column, sortdir: sortDirection, match: query, isActive: filter })
      .then((res) => {
  ...
  const handleDelete = (e) => {
    ...
    deleteLanguage(removeId)
      .then(() => { setModalDelete(false); fetchItems(); toast.success("Language removed successfully!"); })
      .catch(() => toast.error("Failed to remove language. Please try again."))
      .finally(() => setIsDeleteLoading(false));
  };

  const col = [
    { name: "Sr No", selector: (_, index) => index + 1, maxWidth: "60px" },
    { name: "Order", selector: (row) => row.order, sortable: true, sortField: "order", maxWidth: "100px" },
      ...
      selector: (row) => (
        <div className="d-flex gap-2">
          {currentPagePermissions.read && (
            <button className="btn btn-sm btn-info" onClick={() => navigate(`/language/${row._id}`)}>View</button>
          )}
          {currentPagePermissions.edit && (
            <button className="btn btn-sm btn-success" onClick={() => navigate(`/language/${row._id}/edit`)}>Edit</button>
          )}
  ...
  document.title = `Language | ${adminData?.companyName}`;

  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb maintitle="Master" title="Language" pageTitle="Master" />
          <Row>
            <Col lg={12}>
              <Card>
                <CardHeader>
                  <FormsHeader
                    ...
                    tog_list={() => navigate("/language/add")}
                    setQuery={setQuery}
